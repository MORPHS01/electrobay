          ...
          {cartItems.length > 0 ? (
            <div className="flex flex-col gap-5">
              {cartItems.map((cartItem, i) => (
                <Link 
                  key={i} 
                  href={{pathname: `/categories/${cartItem.category}/${cartItem.productName}`, query: { productInfo: JSON.stringify(cartItem)}}}
                ...
                >
                  <div className="w-[27%] max-md:w-[30%] h-full">
                    <Image src={cartItem.productImage} alt={cartItem.productName} width={1000} height={1000} className="object-contain h-full"/>
                  </div>

                  <div className="flex flex-col h-full justify-between flex-1">
                    <span className="flex flex-col gap-[1px]">
                      <h1 className="font-semibold font-poppins text-xl max-md:text-lg group-hover:text-blue-600 dark:group-hover:text-blue-400">{cartItem.productName}</h1>
                      <p className="line-clamp-1 max-md:line-clamp-2 text-gray-600 dark:text-gray-400 text-sm">{cartItem.productDescription}</p>
                    </span>
                    <p className="text-lg font-semibold text-blue-600 dark:text-blue-400"><span className="mr-[3px]">₦</span>{cartItem.productPrice.toLocaleString()}</p>
                        ...
                        <span className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                          <Icon
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              e.preventDefault();
                              setCartItems(prev => prev.filter(item => item.productId !== cartItem.productId))
                            }}
                          />
                        </span>
                      </div>
        ...
        {cartItems.length > 0 && (
          <aside className="w-[30%] max-lg:w-[35%] max-md:hidden h-fit sticky top-[200px]">
            <div className="px-[24px] py-[48px] w-full flex flex-col gap-[16px] bg-white dark:bg-[#0F1125] rounded-lg border border-[#E5E5E5] dark:border-[#181C3A]">
              <span className="flex justify-between">
                <p>Subtotal</p>
                <p><span className="mr-[3px]">₦</span>{subTotal.toLocaleString()}</p>
              </span>
              <hr className="w-full border border-[#E5E5E5] dark:border-[#181C3A]"/>

              <span className="flex justify-between">
                <p>Shipping Fee</p>
                <p><span className="mr-[3px]">₦</span>{shippingFee.toLocaleString()}</p>
              </span>
              <hr className="w-full border border-[#E5E5E5] dark:border-[#181C3A]"/>
              ...
              <span className="flex justify-between">
                <p>Total</p>
                <p><span className="mr-[3px]">₦</span>{total.toLocaleString() }</p>
              </span>

              <Link 
                href={{pathname: "/checkout", query: { totalAmount: total}}} 
            ...
            <div className="mt-8 flex justify-end">
              <Button type="fill" bgColor="#FF0000" bgHover="#FF4747" scaleOnHover={false} onClick={() => setCartItems([])}>Clear All</Button>
            </div>
          </aside>
        )}
        ...
        {cartItems.length > 0 && (
          <aside className="hidden max-md:block w-full mb-10">
            <div className="px-[20px] py-[20px] w-full flex flex-col gap-[13px] bg-white dark:bg-[#0F1125] rounded-lg border border-[#E5E5E5] dark:border-[#181C3A]">
              <span>
                <h1 className="font-poppins font-semibold text-lg mb-2 text-center">CART SUMMARY</h1>
                <hr className="w-full border border-[#E5E5E5] dark:border-[#181C3A]"/>
              </span>
              ...
              <article id="bottomFixed" className="fixed z-[1000] bottom-0 left-0 w-full px-[10%] py-5 bg-white dark:bg-[#0F1125] rounded-t-3xl border border-[#E5E5E5] dark:border-[#181C3A]">
                <section className="flex justify-between items-center h-full ">
                  <Link href={{pathname: "/checkout", query: { totalAmount: total}}} className="inline-block px-6 py-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors text-center">
                    Proceed to Checkout <span className="font-semibold"><span className="ml-[5px] mr-[3px]">₦</span>{total.toLocaleString()}</span>
                  </Link>
