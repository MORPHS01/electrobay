  ...
  return (
    <main className="sticky z-[10000] -top-[25px] flex justify-center">
      <nav className="absolute top-[32px] flex flex-col gap-3 shadow-md shadow-black/20 dark:shadow-[#78B1E4]/20 items-center w-[85%] max-sm:w-[95%] backdrop-blur-sm bg-white/75 dark:bg-[#000022]/85 rounded-lg max-md:rounded-b-none py-[14px] max-md:py-[10px] max-sm:py-[6px]">
          ...
          <section className="flex gap-3 items-center">
            ...
            <div ref={userProfileRef} className="relative">
              ...
              >
                {image ? (
                  <Image
                    ...
                    width={25}
                    height={25}
                    className="rounded-full w-[27px] max-md:w-[22px] aspect-square border-[2px] border-[#cbcbcb] dark:border-[#181C3A]"
                  />
                ) : (
                  <Icon name="user" />
                )}
              ...
              {name ? (
                ...
                <LoggedOut visible={isUserProfileVisible} />
              )}
            </div>
        ...
        {isNotHomePage && (
          <hr className="w-full border border-[#E5E5E5] dark:border-[#181C3A] max-md:hidden" />
        )}
        ...
        {isNotHomePage && (
          <div className="flex justify-between w-full px-8 max-md:hidden">
            {categories.map((category, index) => (
              <Link
                key={index}
                href={`/categories/${category.toLowerCase().trim()}`}
                className="first:hidden text-sm text-[#000011] dark:text-[#F4F9FD] hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-300 ease-in-out"
              >
                {category}
              </Link>
            ))}
