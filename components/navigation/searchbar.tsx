    ...
    selectedCategory !== "All Products"
      ? filteredItems.filter((item) =>
          selectedCategory
            .trim()
            .toLowerCase()
            .includes(item.category.trim().toLowerCase())
        )
      : filteredItems;
  ...
  return (
    <main className="w-[50%] flex items-stretch max-md:hidden">
      <section
        ...
        className="relative min-w-fit rounded-l-[8px] cursor-pointer bg-[#f0f0f0] dark:bg-[#0F1125] px-[1rem] py-[0.6rem] flex items-center gap-2"
      >
        <Icon name="menu" noHover />
        <p>{selectedCategory}</p>
        {/* Dropdown */}
        <div
          className={`absolute left-0 top-full rounded-[8px] transform transition-all duration-200 ease-out ${
            isDropdownOpen
          ...
          <ul className="max-h-[250px] mt-1 min-w-[200px] w-full bg-[#f0f0f0] dark:bg-[#0F1125] border border-[#2A2B2A55] dark:border-[#181C3A] rounded-[8px] overflow-x-hidden">
            {categories.map((category, i) => (
              <li
                ...
                className={`px-4 py-3 cursor-pointer hover:bg-[#e6e6e6] dark:hover:bg-[#12152F] border-b border-[#2A2B2A11] dark:border-[#181C3A] transition-all duration-250 ease-in-out ${
                  selectedCategory === category &&
                  "bg-[#e6e6e6] dark:bg-[#12152F]"
                }`}
              >
                {category}
              </li>
      ...
      <section className="flex-1 relative">
        <input
          ...
          className="outline-none rounded-r-[8px] px-[1rem] w-full h-full bg-[#e6e6e6] dark:bg-[#12152F]"
        />
        <div className="absolute w-fit h-[90%] top-1/2 -translate-y-1/2 right-[1rem] flex items-center justify-center bg-[#e6e6e6] dark:bg-[#12152F]">
          <Link
            href={{ pathname: "/search-results", query: { query: query } }}
            onClick={() =>
              ...
              {categoryFilter.length ? (
                categoryFilter.map((item, i) => (
                  <Link
                    key={i}
                    href={{
                      pathname: `/categories/${item.category}/${item.productName}`,
                      query: { productInfo: JSON.stringify(item) },
                    }}
                    onClick={() => {
...
export function MobileSearchBar({ categories }: { categories: string[] }) {
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === "Enter") {
      setTimeout(() => setIsSearchOpen(false), 140);
      const queryString = new URLSearchParams(mobileQuery)
        .toString()
        .replace("=", "");
      router.push(`/search-results?query=${queryString}`);
      localStorage.setItem("searchResults", JSON.stringify(categoryFilter));
  ...
  // Search Algorithm
  const items = data;
  const [mobileQuery, setMobileQuery] = useLocalStorage(
    "mobileSearchQuery",
    ""
  );
  const [isSearchOpen, setIsSearchOpen] = useState(false);
  const filteredItems = useMemo(
    () =>
      items.filter((item) =>
        item.productName.toLowerCase().includes(mobileQuery.toLowerCase())
      ),
    [mobileQuery, items]
  );
  const categoryFilter =
    ...
    <main className="hidden max-md:flex items-stretch w-full absolute top-full rounded-b-[8px] shadow-md shadow-black/20 dark:shadow-[#78B1E4]/20">
      ...
      <section className="flex-1 relative">
        <input
          ...
          onBlur={() => setTimeout(() => setIsSearchOpen(false), 140)}
          placeholder="I'm Searching for..."
          className="outline-none rounded-br-[8px] px-[1rem] w-full h-full bg-[#e6e6e6] dark:bg-[#12152F]"
        />
        <div className="absolute w-fit h-[90%] top-1/2 -translate-y-1/2 right-[1rem] flex items-center justify-center bg-[#e6e6e6] dark:bg-[#12152F]">
          <Link
            href={{
              pathname: "/search-results",
              query: { query: mobileQuery },
            }}
            onClick={() =>
              localStorage.setItem(
                "searchResults",
